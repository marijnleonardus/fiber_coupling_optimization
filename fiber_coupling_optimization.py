...
# Desired power of fiber coupling for optimization in units of mW. Entered by user.
desired_power = 30
...
def read_output():
    ...
    task.CreateAIVoltageChan(
        'Dev3/AI0', "", DAQmx_Val_RSE, 0, 2, DAQmx_Val_Volts, None)
    task.StartTask()
    task.ReadAnalogF64(
        num_samples,
        -1,
...
def move_one_motor_rel(motor, motor_displacement):
    ...
    Args:
        motor (int): which motor to use
        motor_displacement (int): steps
    """
...
def move_one_motor_abs(motor, motor_target_position):
    ...
    Args:
        motor (int): _description_
        motor_target_position (float): detination in steps
    """
...
def move_two_motors_rel(motor1, motor2, motor1_displacement, motor2_displacement):
    ...
    Args:
        motor1 (int): which motor to use
        motor2 (int): which motor to use
        motor1_displacement (int): relative displacement
        motor2_displacement (int): relative displacement
    """
...
def move_two_motors_abs(motor1, motor2, motor1_target_position, motor2_target_position):
    ...
    Args:
        motor1 (int): id of motor 1
        motor2 (int): id of motor 2
        motor1_target_position (float): position of motor 1
        motor2_target_position (float): position of motor 2
    """
...
def move_four_motors_abs(motor1, motor2, motor3, motor4, motor_positions):
    ...
    Args:
        motor1 (int): motor id 1
        motor2 (int): motor id 2
        motor3 (int): motor id 3
        motor4 (int): motor id 4
        motor_positions (np.ndarray): array of motor destinations
    """
...
def set_motor_home(motor):
    ...
    Args:
        motor (int): the motor to return to home position
    """
...
def randomize_initial_simplex(motor1, motor2, motor3, motor4, simplex_range):
    ...
    Args:
        motor1 (int): nr of motor
        motor2 (int): 
        motor3 (int): 
        motor4 (int): 
        simplex_range (float): range to search for from + to - for the given range
    """
...
def order(positions, output_positions):
    ...
    Args:
        ...
        output_positions (_type_): _description_
    """
...
def centroid():
    """Solves for the centroid of the simplex excluding the worst position. 
    Used in elements of downhill_simplex and optimize function.
    """
...
def reflection(motor1, motor2, motor3, motor4):
    ...
    reflection_position = centroid_position + \
        1 * (centroid_position - worst_position)
...
def expansion(motor1, motor2, motor3, motor4):
    """Solves for a position expanded from the reflected position. 
    Used in elements of downhill_simplex and optimize function.
    ...
    expansion_position = centroid_position + 2 * \
        (reflection_position - centroid_position)
...
def contraction(motor1, motor2, motor3, motor4):
    """Solves for a position contracted inside the simplex.
    Used in elements of downhill_simplex and optimize function.
    ...
    contraction_position = centroid_position + \
        0.5 * (worst_position - centroid_position)
...
def shrink(motor1, motor2, motor3, motor4):
    """Solves for all new simplex positions shrunk toward the current best position.
    Used in elements of downhill_simplex and optimize function.
    ...
    shrink_position2 = best_position + 0.5 * \
        (np.array(simplex[1]) - best_position)
    move_four_motors_abs(motor1, motor2, motor3, motor4, shrink_position2)
    read_output()
    shrink_output2 = average_output()
    shrink_position3 = best_position + 0.5 * \
        (np.array(simplex[2]) - best_position)
    move_four_motors_abs(motor1, motor2, motor3, motor4, shrink_position3)
    read_output()
    shrink_output3 = average_output()
    shrink_position4 = best_position + 0.5 * \
        (np.array(simplex[3]) - best_position)
...
def correct_hysteresis(motor1, motor2):
    """Corrects hysteresis through a process of moving two motors relatively in an output increasing direction. 
    Used in optimize function when local max is found.
    Corrected motors should usually the two horizontals together and two verticals together.

    Args:
        motor1 (int): nr of motor
        motor2 (int): nr of motor
    """
    motor1_motion = 50
    motor2_motion = 50
    counter = 0
...
def explore_motor(motor, direction):
    """Explores one motor in one direction 2000 steps by moving this far and working back 
    to the original position 100 steps at a time. 
    Used in optimize function.

    Args:
        motor (int): motor to be explored
        direction (int): -1 (backward) or 1 (forward)
    """
    explore_step = 100
    if direction < 0:
...
def optimize(motor1, motor2, motor3, motor4, desired_power):
    """Optimizes four motors and finds a local maximum of power.
    Begins with intializing a simplex and performing downhill simplex. 
    
    After three iterations of same best postion this function corrects for hystersis and
    prompts the user to decide if they would like to continue optimizing. 
    
    If a low ouput is found, optimizer will explore to find the global peak instead.
     
    Optimization with stop if desired power is achieved.

    Args:
        motor1 (int): nr of motor. horizontal mirror
        motor2 (int): nr of motor. vertical mirror
        motor3 (int): nr of motor. horizontal mirror
        motor4 (int): nr of motor. vertical mirror
        desired_power (float): desired power output in mW
    """
    ...
    while deadline > time.time():
        ...
        if final_position == prev_best_position:
            ...
            if hysteresis_counter > 2:
                hysteresis_counter = 0
                move_four_motors_abs(
                    motor1, motor2, motor3, motor4, simplex[-1])
                ...
                if final_output > desired_power:
                    ...
                    randomize_initial_simplex(
                        motor1, motor2, motor3, motor4, 1000)
                    order(simplex, output_simplex)
                else:
                    print('Exploring...')
                    explore = True
                    ...
                    while explore == True:
                        ...
                        if exploring_motor > 4:
                            ...
                            input(
                                'Optimizer may be stuck or output is too low. Couple manually to better output and press enter to restart optimization.')
                            explore = False
